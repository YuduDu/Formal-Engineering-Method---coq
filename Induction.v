...
Theorem plus_n_Sm : forall n m : nat, 
  S (n + m) = n + (S m).
Proof. 
  intros n m. induction n as [| n' lHn'].
  -(*n=0*)
    reflexivity.
  - (*n = Sn'*)
    simpl. rewrite lHn'. reflexivity.
Qed.
...
Proof.  
  intros n. induction n as [|n' lHn'].
  - reflexivity.
  - simpl. rewrite lHn'. rewrite -> plus_n_Sm. reflexivity.
Qed. 
...
'induction' provides a proof by establishing a hypothesis holds for a small
number of base cases. It generates a subgoal for each constructor of the given
term, and makes the induction hypothesis available in the current context.
'destruct' provides simple case analysis. It generates a goal for each
constructor of the given term but does not provide an induction hypothesis.
...
Proof.
  ...
  rewrite -> H. 
  assert(L: m+(n+p) = (n+p)+m). { rewrite->plus_comm.  reflexivity. }
  rewrite -> L. simpl. rewrite -> plus_assoc. reflexivity. Qed.
...
Theorem mult_plus: forall n m : nat,
  m + m * n = m * S n.
Proof.
  intros n m.
  induction m.
...
Proof.
  ...
  - rewrite-> plus_O_n.  simpl. intros H. rewrite->H. reflexivity.
  -  intros H. simpl. rewrite->IHn'. reflexivity.
...
Proof.
 intros n m p. induction n.
- simpl. reflexivity.
-  rewrite->plus_comm. rewrite<-plus_assoc. assert(H: p+S n = S n + p). {rewrite-> plus_comm. reflexivity. }
rewrite->H. reflexivity.
Qed.
...
Theorem bin_incr_nat_plus: forall b:bin,
  bin_to_nat (incr b) = bin_to_nat b +1.
Proof.
intros b. induction b.
- simpl. reflexivity.
-simpl. rewrite->plus_n_Sm. rewrite->plus_n_Sm. rewrite<-plus_n_O. rewrite<-plus_assoc. reflexivity.
- simpl. rewrite<-plus_n_O. rewrite<-plus_n_O. rewrite->plus_n_Sm. rewrite->IHb.  rewrite->plus_swap.
rewrite<-plus_assoc. simpl. rewrite->plus_assoc. reflexivity.
Qed.

Theorem equal_nat_to_bin: forall n:nat,
...
- simpl. destruct  ( nat_to_bin n' ).
  ...
  +rewrite<-IHn'. simpl. reflexivity.
  + rewrite<-IHn'. simpl. rewrite->plus_n_Sm. rewrite->plus_n_Sm. rewrite->plus_n_Sm. rewrite->plus_n_Sm. rewrite->bin_incr_nat_plus.
rewrite<-plus_n_O. rewrite->plus_swap.
rewrite<-plus_assoc. simpl. reflexivity.
Qed.

(** b *)
 (* we can't represent 'zero' in binary because we convert from natural number.*)

(** c *)
Definition direct (n:bin) :bin := nat_to_bin (bin_to_nat n).

Theorem direct_prov: forall n:bin,
  nat_to_bin (bin_to_nat n) = direct n.
Proof.
  intros n.
  induction n.
  -simpl. reflexivity.
...
(** $Date: 2016-01-07 09:17:43 -0500 (Thu, 07 Jan 2016) $ *)
