...
(**  Drat: The infix syntax for [or] is also what we use for
   "evaluates to" later on, so it is automagically turned into a
   down-arrow symbol... :-(  FIX: Use \eval instead for the later one. 
 *)
Infix "&&" := andb.
Infix "||" := orb.
...
(** The [Check] command asks Coq to print the type of an
...
Fixpoint bin_to_nat (n : bin): nat :=
  match n with 
    | Zero => 0
    | T n' => (bin_to_nat n') * 2
    | oneMoreT n' => (bin_to_nat n') * 2 +1
  end.

Example test_bin_incr1: (bin_to_nat (incr (T(oneMoreT(Zero))))) = 3.
Proof. simpl. reflexivity. Qed.
Example test_bin_incr2: (bin_to_nat (incr (T(T(oneMoreT(Zero)))))) = 5.
Proof. simpl. reflexivity.  Qed.
(* FILL IN HERE *)
(** [] *)
